...
\title{Die 7 Ringe der \ceva{c-base}}
\subtitle{Eine interpretative Exegese der ältesten Überlieferung im Lichte neuer Ausgrabungen und Schlussfolgerungen zur Topologie der Station vor ihrer Faltung}
...
\begin{frame}{\cevain{QR}}
    \includegraphics[height=0.618\textheight]{qr-7ringe.png}
    
\end{frame}

\begin{frame}{Aktueller Befund / kanonische Darstellung}
    \resizebox{!}{0.8\textheight}{
        \input{tikz-c-base}
        }
\end{frame}

\begin{frame}{Periodisierung der Rekonstruktionsgeschichte}
    \resizebox{0.9\textwidth}{!}{
    \input{tikz-zeitstrahlzwei}
    % \input{tikz-periodisierung}
    }
\end{frame}

\begin{frame}{Die kanonischen Schriften / \ceva{c\_rift})}
    \resizebox{\textwidth}{!}{
        \begin{tabular}{r|lrl}
        \toprule
...
\begin{frame}[allowframebreaks]{\ceva{c\_rift}}
...
\begin{frame}{Besondere \ceva{ringe}}
    \begin{itemize}
        \item Verbindung von \ceva{core} und \ceva{clamp}
        \item Sonderstellung von \ceva{creactive} als
        \item[] \cevain{most powerful part}
    \end{itemize}

    \vspace{2ex}

    Welche Topolgie ist \cevain{urcprünglich}?
\end{frame}

\begin{frame}{Verflachung}

    \begin{itemize}
        \item Aktuelle Form Folge des Absturzes (\ceva{abcturc})
        \item Ursprüngliche Form komplexer (\ceva{complex})
        \item Verbindung von \ceva{core} und \ceva{clamp} (ER-Brücke)
        \item Besondere Rolle von \ceva{creactive}
    \end{itemize}

\end{frame}

\begin{frame}[allowframebreaks]{Ringförmige Anordnung der \ring{ringe}}
    \resizebox{!}{0.8\textheight}{
     \input{tikz-ringring}
    }
    \framebreak
    \resizebox{!}{0.8\textheight}{
     \input{tikz-ringe-olympisch}
    }
...
\begin{frame}{Villarcreau-Kreise}
...
\begin{frame}{Villarcreau-\ceva{ringe}}
...
\begin{frame}{\ring{Ringe} in sich geschossen?}
    \begin{itemize}
        \item keine ausschließenden Kategorien
        \item \ceva{c\_rift} betont Übergänge
    \end{itemize}
\end{frame}

\begin{frame}[allowframebreaks]{Torusknoten (\ceva{cnoten})}
...
\begin{frame}[allowframebreaks]{Gefärbter Torusknoten}
    ...
    \begin{itemize}
        \item Torus einteilen in Flächen (Module)
        \item Farbe von jeder Fläche anders als angrenzende
    \end{itemize}

    $\rightarrow$ \ceva{c-tation} möglicherweise ein \cevain{mosaic}
...
\begin{frame}{\cevain{mosaic}}
    \includegraphics[height=0.8\textheight]{aufkleber-donut-von-homer-simpson.jpg}
\end{frame}
...
\begin{frame}{Merci}
    ...
    \cevain{c\_prechc\_tunde} 
    \begin{description}
        \item[time:] 
        Setting Orange 42:23 o'clock M.T.
    \\ bitte Voranmeldung: \href{mailto:penta@c-base.org}{penta@c-base.org}
        \item[space:] in the bottom of a locked filing cabinet stuck in a disused lavatory with a sign on the door saying `Beware of the Leopard', local planning department, Alpha Centauri
    \end{description}
